...
```bash
# ✅ 正确
bash skills/wecom-meeting/scripts/wecom-meeting.sh create --title "会议" ...

# ❌ 错误 — 会导致路径计算错误、配置文件找不到
source skills/wecom-meeting/scripts/wecom-meeting.sh create --title "会议" ...
. skills/wecom-meeting/scripts/wecom-meeting.sh create --title "会议" ...
```
...
### 创建会议
```bash
bash skills/wecom-meeting/scripts/wecom-meeting.sh create --title "标题" --start "YYYY-MM-DD HH:MM" [--duration 3600] [--description "描述"] [--invitees "user1,user2"] [--hosts "host1"] [--password "1234"]
```

### 修改会议
```bash
bash skills/wecom-meeting/scripts/wecom-meeting.sh update --meetingid "ID" [--title "新标题"] [--start "新时间"] [--duration 7200] [--invitees "user1"]
```
- `meetingid` 必需；不传的参数保持原值

### 取消会议
```bash
bash skills/wecom-meeting/scripts/wecom-meeting.sh cancel "meetingid"
```

### 查询会议详情
```bash
bash skills/wecom-meeting/scripts/wecom-meeting.sh info "meetingid"
```

### 列出用户会议（获取成员会议ID列表）
```bash
bash skills/wecom-meeting/scripts/wecom-meeting.sh list "userid" ["开始时间"] ["结束时间"]
```
