...
`default_calendar_color` 可在 `config.json` 中全局设置，优先级：函数调用传参 > 环境变量 `DEFAULT_CALENDAR_COLOR` > config.json > 硬编码默认值 `#3366CC`。
...
```bash
# ✅ 正确
bash skills/wecom-schedule/scripts/wecom-schedule.sh create --title "会议" ...

# ❌ 错误 — 会导致路径计算错误、配置文件找不到
source skills/wecom-schedule/scripts/wecom-schedule.sh create --title "会议" ...
. skills/wecom-schedule/scripts/wecom-schedule.sh create --title "会议" ...
```
...
| 操作 | 命令 | 关键参数 |
|------|------|---------|
| 创建日历 | `./wecom-schedule.sh create-calendar "名称" ["描述"] ["颜色代码"]` | 返回 `cal_id`；颜色示例：`#3366CC`(蓝) `#FF6600`(橙) `#009900`(绿)，默认 `#3366CC` |
